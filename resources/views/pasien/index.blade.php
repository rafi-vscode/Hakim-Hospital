<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                <a href="{{ route('pasien.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300">
                    + Tambah Pasien Baru
                </a>
            </div>
                ...
                <form method="GET" action="{{ route('pasien.index') }}">
                    <div class="flex">
                        <input type="text" name="search"
                               ...
                               placeholder="Cari berdasarkan nama, no. rekam medis, atau status..."
                               value="{{ $search ?? '' }}">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">No. Rekam Medis</th>
...
@if ($pasien->foto_profil)
    <img src="{{ asset('storage/' . $pasien->foto_profil) }}" width="120">

@else
    <div class="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center text-xs text-gray-500 border border-dashed">
        No Foto
    </div>
@endif
</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <!-- Styling Status Pasien -->
                                        <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                            @if($pasien->status == 'Darurat') bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100
                                            @elseif($pasien->status == 'Rawat Inap') bg-yellow-100 text-yellow-800 dark:bg-yellow-700 dark:text-yellow-100
                                            @elseif($pasien->status == 'Rawat Jalan') bg-blue-100 text-blue-800 dark:bg-blue-700 dark:text-blue-100
                                            @else bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100
                                            @endif">
                                            {{ $pasien->status }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $pasien->alamat }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <!-- Link untuk Edit -->
                                        <a href="{{ route('pasien.edit', $pasien->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300">Edit</a>
                                        
                                        <!-- Tombol Hapus (sebagai Form) -->
                                        <form class="inline" method="POST" action="{{ route('pasien.destroy', $pasien->id) }}" onsubmit="return confirm('Anda yakin ingin menghapus data pasien ini?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                ...
                                <tr>
                                    <!-- Kolom harus 7: Foto(1) + 5 kolom data + Aksi(1) -->
                                    <td colspan="7" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                                        @if ($search)
                                            Data pasien dengan kueri "{{ $search }}" tidak ditemukan.
                                        @else
