        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <!-- 
                      PENTING:
                      - action-nya ke route('rekam-medis.update', $rekamMedis->id)
                      - Kita tambahkan @method('PUT') untuk memberi tahu Laravel ini adalah UPDATE
                    -->
                    <form method="POST" action="{{ route('rekam-medis.update', $rekamMedis->id) }}">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <select name="pasien_id" id="pasien_id" 
                                ...
                                @foreach ($pasiens as $pasien)
                                    <!-- 
                                      Logika 'selected':
                                      Cek old('pasien_id'), jika tidak ada, cek data dari $rekamMedis->pasien_id
                                    -->
                                    <option value="{{ $pasien->id }}" 
                                        {{ old('pasien_id', $rekamMedis->pasien_id) == $pasien->id ? 'selected' : '' }}>
                            ...
                            <select name="layanan_id" id="layanan_id" 
                                ...
                                @foreach ($layanans as $layanan)
                                    <option value="{{ $layanan->id }}" 
                                        {{ old('layanan_id', $rekamMedis->layanan_id) == $layanan->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="tanggal_kunjungan" id="tanggal_kunjungan" 
                                   value="{{ old('tanggal_kunjungan', $rekamMedis->tanggal_kunjungan) }}"
                                   class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600" required>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="catatan_dokter" id="catatan_dokter" rows="4" 
                                      class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600">{{ old('catatan_dokter', $rekamMedis->catatan_dokter) }}</textarea>
