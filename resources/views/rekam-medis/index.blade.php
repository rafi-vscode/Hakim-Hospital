<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                <a href="{{ route('rekam-medis.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300">
                    + Tambah Catatan Baru
                </a>
            </div>
                ...
                <form method="GET" action="{{ route('rekam-medis.index') }}">
                    <div class="flex">
                        <input type="text" name="search"
                               ...
                               placeholder="Cari berdasarkan nama pasien atau layanan..."
                               value="{{ $search ?? '' }}">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Layanan Diberikan</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Tanggal Kunjungan</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Catatan Dokter</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <!-- 
                              Looping data rekam medis.
                              Kita panggil relasi ->pasien dan ->layanan
                            -->
                            @forelse ($rekamMedis as $catatan)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                                        {{ $catatan->pasien->nama_pasien ?? 'Pasien Dihapus' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        {{ $catatan->layanan->nama_layanan ?? 'Layanan Dihapus' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        <!-- Format tanggal agar mudah dibaca -->
                                        {{ \Carbon\Carbon::parse($catatan->tanggal_kunjungan)->translatedFormat('d F Y') }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <!-- Link untuk Edit -->
                                        <a href="{{ route('rekam-medis.edit', $catatan->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300">Edit</a>
                                        
                                        <!-- Tombol Hapus (sebagai Form) -->
                                        <form class="inline" method="POST" action="{{ route('rekam-medis.destroy', $catatan->id) }}" onsubmit="return confirm('Anda yakin ingin menghapus catatan ini?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                                        @if ($search)
                                            Catatan rekam medis dengan kueri "{{ $search }}" tidak ditemukan.
                                        @else
